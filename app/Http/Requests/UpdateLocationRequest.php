...
class UpdateLocationRequest extends FormRequest
{
    ...
    {
        $locationId = (string) $this->route('id');
        ...
        return [
            ...
            'code' => ['required', 'string', 'max:30', 'regex:/^[A-Z0-9\-]+$/', function ($attribute, $value, $fail) use ($locationId) {
                $warehouseId = $this->input('warehouse_id');
                if (Location::where('code', $value)->where('warehouse_id', $warehouseId)->where('_id', '!=', $locationId)->exists()) {
                    $fail('Ya existe otra ubicación con ese código en este almacén.');
                }
            }],
